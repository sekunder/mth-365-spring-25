...
We first divide the predictor space into *J* distinct overlapping regions. We predict that each observation belongs to the most commonly occurring class of training observations in the region to which it belongs.
...
We first divide the predictor space into *J* distinct overlapping regions. Then for every observation that falls into the region $R_j$, we make the same prediction, which is simply the mean of the response values for the training observations in $R_j$.
