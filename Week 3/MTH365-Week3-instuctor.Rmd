...
```{r, fig.align='center'}
ggplot(hate_crimes, aes(y=median_income, x = region)) +
  geom_boxplot()
...
(a). The data set `Weather` contains data on weather-related variables for several world cities.
...
(b). We want to study the average temperature. First, we want to see its distribution. How would we do this?
...
(c). Next, we want to study the distribution of average temperature across different cities. How would we do this?
...
(d). The average temperature may be related to date. How to show the relation between temperature and date?
...
(e). Maybe different city has totally different trend on average temperature (ie. relationship between temperature and date grouped by city). 
...
(f). What if we only care about one city? Show the relationship between temperature and date for Beijing only.
...
(g). Instead of the date, we can also use the month. Show the relationship between month and temperature (for Beijing). 
...
(h). Show the distribution of average temperature for the Beijing data set by month.

```{r}
ggplot(Beijing, aes(x=avg_temp)) + 
...
(i). Show the relationship between the low temperature and the high temperature colored by month
